...
if isdir(output):
    print(f"Output directory {output} exists already")
    exit(0)
else:
    mkdir(output)

for filename in sorted(listdir(input)):
    if filename != ".DS_Store":
        file_out = filename.replace('.tex', '.md')
        file_input = f"{input}/{filename}"
        file_output = f"{output}/{file_out}"
        print(f"==================\nFILE {filename}\n==================")
